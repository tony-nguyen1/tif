...
export async function getSet(userId: string, exerciseId: number): Promise<{ exercise: table.Exercise | null, set: table.Set | null }[]> {
    const res: { exercise: table.Exercise | null, set: table.Set | null }[] = await db
        ...
        .where(and(
            eq(table.exercise.userId, userId),
            eq(table.exercise.id, exerciseId)));
...
export async function getSetBis(userId: string, exerciseId: number): Promise<WorkoutWithExercise[]> {
...
export async function getAnExercise(exerciseId: number): Promise<table.Exercise | null> {
    const res: table.Exercise | undefined = await db.query.exercise.findFirst({ where: eq(table.exercise.id, exerciseId) });
    return res ?? null;
}
...
export async function getLastSeries(workoutId: number): Promise<{ workout: table.Workout, set: table.Set | null }> {
